...
"""Train a Farm C supervised 1D-CNN window classifier.

The script:
1. Reads Wind Farm C metadata and one CSV per Event.
2. Splits Events before any window is generated.
3. Selects features using training Events only.
4. Creates 24-hour windows (144 points at 10-minute resolution).
5. Uses moderately denser anomalous training windows and a configurable
   normal-to-anomaly sampling ratio.
6. Uses one common, label-independent stride for validation and test Events.
7. Selects the classification threshold using validation Events only.
8. Trains a smaller regularised 1D-CNN and reports window-level metrics only.
9. Does not aggregate window predictions into Event-level decisions.
"""
...
def build_model(window_size: int, n_features: int, learning_rate: float):
    """Build a smaller 1D-CNN for improved cross-Event generalisation."""
...
def run(cfg: Config) -> None:
    ...
    for fold, (outer_train_idx, test_idx) in enumerate(splitter.split(X_placeholder, y_event, groups), start=1):
        ...
        model = build_model(cfg.window_size, len(features), cfg.learning_rate)
        callbacks = [
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train Event-grouped Farm C 1D-CNN.")
