...
"""Train a Farm C supervised CNN-GRU window classifier.

This script keeps the same leakage-safe experimental protocol used by the final
1D-CNN and TCN comparisons: Event-grouped outer folds, training-only feature
selection and scaling, identical 24-hour window construction, explicit training
class sampling, validation-only FAR-constrained threshold selection, and
window-level out-of-fold evaluation. Only the neural architecture is changed.

The hybrid network first uses compact Conv1D blocks to extract local SCADA
patterns and reduce sequence length, then applies a GRU to model the order and
longer temporal dependencies of the extracted features. No Event-level
...
def build_model(
    ...
    """Build a compact CNN-GRU for strict cross-Event generalisation."""
    tf = require_tensorflow()
    regularizer = tf.keras.regularizers.l2(model_l2)
    ...
    # Sequential modelling after convolutional compression. recurrent_dropout is
    # intentionally zero to retain the fast fused GPU implementation when present.
    ...
    x = tf.keras.layers.Dropout(dense_dropout, name="dense_dropout")(x)
    ...
    model = tf.keras.Model(inputs, outputs, name="farm_c_cnn_gru")
    model.compile(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train Event-grouped Farm C CNN-GRU.")
    ...
    parser.add_argument("--cnn-filters-1", type=int, default=32)
    parser.add_argument("--cnn-filters-2", type=int, default=64)
    parser.add_argument("--cnn-kernel-size-1", type=int, default=5)
    parser.add_argument("--cnn-kernel-size-2", type=int, default=3)
    parser.add_argument("--gru-units", type=int, default=32)
    parser.add_argument("--conv-dropout", type=float, default=0.20)
    parser.add_argument("--gru-dropout", type=float, default=0.20)
    parser.add_argument("--dense-dropout", type=float, default=0.40)
    parser.add_argument("--model-l2", type=float, default=5e-4)
