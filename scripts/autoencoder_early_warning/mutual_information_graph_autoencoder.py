...
def parse_args() -> Config:
    ...
    for k, v in vars(ns).items():
        if hasattr(cfg, k) and k not in {"datasets_dir","metadata_file","feature_description_file","output_dir","graph_method"}:
            setattr(cfg, k, v)
    return cfg
...
def process_event(row,cfg,fd):
    ...
    model.save_weights(out/"graph_autoencoder.weights.h5"); pd.DataFrame(hist.history).to_csv(out/"training_history.csv",index=False)
    ...
    pd.DataFrame(cal["calibration_candidates"]).to_csv(out/"threshold_calibration_candidates.csv",index=False)
...
def aggregate(results,out,method):
...
def main():
    ...
    if results:
        M=aggregate(results,out,cfg.graph_method); print(json.dumps(M,indent=2))
    pd.DataFrame(failures).to_csv(out/"failed_events.csv",index=False); print("COMPLETE",len(results),"successful,",len(failures),"failed")
